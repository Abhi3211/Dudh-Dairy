...
import { IndianRupee, Milk, Package, TrendingUp, TrendingDown, Truck, HandCoins, AlertCircle } from "lucide-react";
...
import { getDashboardSummaryAndChartData } from "./actions";
...
export default function DashboardPage() {
  ...
  const { firebaseUser, authLoading, userProfile, companyProfile, profilesLoading } = useUserSession(); // Added companyProfile, profilesLoading
  ...
  const fetchData = useCallback(async () => {
    if (authLoading || profilesLoading || !firebaseUser || !companyId) { // Check for companyId and profilesLoading
      setIsLoading(false);
      if (!authLoading && !firebaseUser) {
        setDisplayedDateRangeString("Please log in to view data.");
      } else if (!profilesLoading && !companyId) {
        setDisplayedDateRangeString("Company information not found. Unable to load dashboard.");
      }
      setSummary(null);
      setChartData([]);
      return;
    ...
    console.log(`CLIENT: Fetching data for range: ${startDate.toISOString()} to ${endDate.toISOString()} for company: ${companyId}`);
    try {
      // Pass companyId to the server action
      const data: DashboardData = await getDashboardSummaryAndChartData(startDate, endDate, companyId); 
    ...
    if (filterType === 'daily' && format(startDate, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') ) {
      ...
      periodPrefix = `for the selected ${filterType}`;
    }
    ...
    setIsLoading(false);
  }, [calculateDateRange, filterType, authLoading, firebaseUser, companyId, profilesLoading]); // Added companyId, profilesLoading
  ...
  useEffect(() => {
    ...
    if (!authLoading && firebaseUser && companyId) { // Ensure companyId is present before fetching
        fetchData();
        ...
        setIsLoading(false);
        setDisplayedDateRangeString("Please log in to view data.");
    } else if (!authLoading && !profilesLoading && firebaseUser && !companyId) {
        setSummary(null);
        setChartData([]);
        setIsLoading(false);
        setDisplayedDateRangeString("Company information is missing. Cannot load dashboard.");
    }
  }, [fetchData, filterType, customStartDate, customEndDate, firebaseUser, authLoading, companyId, profilesLoading]); // Added companyId, profilesLoading
  ...
  if (authLoading || profilesLoading) { // Check profilesLoading as well
    return <div className="flex justify-center items-center min-h-screen"><p>Loading user session...</p></div>;
  }

  if (!firebaseUser) {
     // Redirect logic is already in the useEffect hook
     return <div className="flex justify-center items-center min-h-screen"><p>Redirecting to login...</p></div>;
  }
  
  if (!companyId && !profilesLoading && firebaseUser) {
     return (
      <div>
        <PageHeader title={pageSpecificTitle} description="Company information is missing." />
         <Card className="border-destructive">
          <CardHeader>
            <CardTitle className="flex items-center text-destructive"><AlertCircle className="h-5 w-5 mr-2" />Unable to Load Dashboard</CardTitle>
          </CardHeader>
          <CardContent>
            <p>Your user profile is not associated with a company. The dashboard cannot be displayed.</p>
      ...
      <Card className="mb-6">
        ...
        <CardContent className="flex flex-col sm:flex-row gap-4 items-end">
          <div className="flex-1 min-w-[150px]">
            ...
            <Select value={filterType} onValueChange={setFilterType} disabled={isLoading || !companyId}>
